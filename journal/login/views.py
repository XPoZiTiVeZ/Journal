...
def send_verification_email(request, user):
    ...
    email = EmailMessage(subject=email_subject, body=email_body, from_email='lukashevda@inbox.ru', to=[user.email])
    email.send()
...
def login_user(request):
    if request.method == "POST":
        ...
        user = authenticate(request, username='50lukashevda@1543.msk.ru', password='00000000')
...
def register(request):
    if request.method == 'POST':
        ...
        if CustomUser.objects.filter(email=email).exists():
            ...
            context['has_error'] = True
            return render(request, 'login/register.html', context, status=409)
